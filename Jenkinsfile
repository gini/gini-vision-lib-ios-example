pipeline {
  ...
  stages {
    ...
    stage('Unit tests') {
      steps {
        sh 'xcodebuild test-without-building -workspace GiniVisionExample/GiniVisionExample.xcworkspace -scheme "GiniVisionExample" -destination \'platform=iOS Simulator,name=iPhone 6\' | /usr/local/bin/xcpretty -c'
      }
    }
    stage('HockeyApp upload') {
      ...
      steps {
        ...
        sh 'xcodebuild -workspace GiniVisionExample/GiniVisionExample.xcworkspace -scheme GiniVisionExample -configuration Release archive -archivePath build/GiniVisionExample.xcarchive client_id=${CLIENT_ID} client_password=${CLIENT_PASSWORD} | /usr/local/bin/xcpretty -c'
